...
// https://chuck.cs.princeton.edu/doc/examples/hid/mouse.ck
...
// abran la ventana
// Window -> Device Browser -> Human Interface Devices
// para ver sus posibilidades, en mi computador
// tengo raton y teclado
...
if (!dispositivo.openMouse(indiceDispositivo)) {
...
// bucle infinito
while( true ) {
    // esperar hasta recibir info del raton
    dispositivo => now;
    
    // mientras recibimos un mensaje
    while( dispositivo.recv(mensaje)) {
        // si el raton se esta moviendo
        if (mensaje.isMouseMotion()) {
            if (mensaje.deltaX) {
                <<< "movimiento en eje X" >>>;
            }
            if (mensaje.deltaY) {
